...
public:
	CCapuccino(CapuccinoType type = CapuccinoType::Regular)
		:CCoffee(std::string("Capuccino") + (type == CapuccinoType::Double ? " (double)" : "")), m_type(type)
	{}
...
public:
	CLatte(LatteType type = LatteType::Regular)
		: CCoffee(std::string("Latte") + (type == LatteType::Double ? " (double)" : "")), m_type(type)
	{}
...
public:
	CMilkshake(MilkshakeType type = MilkshakeType::Large)
		: CBeverage(std::string("Milkshake (") + MILKSHAKE_TYPE_TO_DATA.at(type).first + ")"), m_type(type)
	{}
