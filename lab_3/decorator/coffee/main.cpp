...
void DialogWithUser()
{
	...
	for (;;)
	{
		...
		{
			beverage = move(beverage) << MakeCondiment<CChocolate>();
		}
		else if (condimentChoice == 5)
		{
			beverage = move(beverage) << MakeCondiment<CLiquor>();
		}
...
int main()
{
				...
				make_unique<CIceCubes>(					// | под нею - кубики льда
					make_unique<CLemon>(				// | | еще ниже лимон
						make_unique<CCinnamon>(			// | | | слоем ниже - корица
							make_unique<CCapuccino>()),	// | | |   в самом сердце - капучино
						2),								// | | 2 дольки лимона
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CTea>(TeaType::Green)	// Берем чай
	...
	{
		auto beverage = 
			make_unique<CCapuccino>()						// Наливаем чашечку капучино,
